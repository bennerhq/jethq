...
// invokeChromeAction reuses the exact callbacks backing the on-video controls,
// ensuring the native macOS menu and control strip never drift in behaviour.
func (a *App) invokeChromeAction(id string) {
	width, height := a.lastWidth, a.lastHeight
	if width < 1 {
	...
	for _, button := range a.layoutChromeButtons(width, height, a.ctrlSnapshot()) {
		if button.id == id && button.enabled && button.onClick != nil {
			button.onClick()
			return
		}
	}
...
func (a *App) ctrlSnapshot() session.Snapshot {
	if a.ctrl == nil {
...
func (a *App) drawStatusFooter(screen *ebiten.Image, snap session.Snapshot) {
	...
	clr := rgba(86, 95, 137, 255, max(alpha, 0.75))
	...
	if snap.LastError != "" && snap.Phase != session.PhaseConnected {
		...
		rightColor = rgba(247, 118, 142, 255, max(alpha, 0.75))
	}
...
func (a *App) drawSettingsOverlay(screen *ebiten.Image, snap session.Snapshot) {
	...
	const (
		sidebarW           = 156.0
		settingsPanelWidth = 920.0
		sideMargin         = 24.0
		bottomMargin       = 24.0
		topFraction        = 0.05
	)
	// Keep the settings window visually stable while navigating sections. Its
	// width is fixed on normal desktop windows; only a narrow host window may
	// constrain it to preserve a usable margin.
	panelW := min(settingsPanelWidth, max(0, float64(bounds.Dx())-sideMargin*2))
	contentW := max(0, panelW-sidebarW-44)
	panelY := float64(bounds.Dy()) * topFraction
	availableH := max(0, float64(bounds.Dy())-panelY-bottomMargin)
	panelH := min(max(a.settingsPanelHeight(section, contentW), settingsSidebarHeight(len(sections))), availableH)
