...
func (e statsOverlayRootElement) Draw(ctx *ui.Context, bounds ui.Rect) {
	// Keep diagnostics visible without obscuring the remote desktop. The panel
	// occupies one fifth of the current window, grows to fit every readout and
	// graph, and is anchored away from the lower-right control strip.
	const margin = 16.0
	const panelInsets = 16.0
	panelW := bounds.W / 5
	if panelW < 1 {
		return
	}
	content := e.child.Measure(ctx, ui.Constraints{MaxW: max(0, panelW-panelInsets*2)})
	panelH := min(content.H+panelInsets*2, bounds.H-margin*2)
	if panelH < 1 {
		return
	}
	panel := ui.Rect{
		X: bounds.X + margin,
		Y: bounds.Bottom() - margin - panelH,
		W: panelW,
		H: panelH,
	}
	e.app.statsPanel = rect{x: panel.X, y: panel.Y, w: panel.W, h: panel.H}
	closeRect := ui.Rect{X: panel.Right() - 38, Y: panel.Y + 8, W: 28, H: 24}
	e.app.statsCloseButton = rect{x: closeRect.X, y: closeRect.Y, w: closeRect.W, h: closeRect.H}
	ui.Panel{
		Fill:   ctx.Theme.ModalFill,
		Stroke: ctx.Theme.ModalStroke,
		Insets: ui.UniformInsets(panelInsets),
		Child:  e.child,
	}.Draw(ctx, panel)
	// Input is handled by App.syncUIPointer so the compact panel can remain a
	// non-interactive readout without forwarding clicks to the remote desktop.
	ui.Button{Label: "X", Enabled: true}.Draw(ctx, closeRect)
}
