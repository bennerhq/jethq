...
type App struct {
	...
	nativeMenuActions      chan string
	nativeMenuInitialized  bool
}
...
func New(cfg Config) (*App, error) {
	...
	// Earlier releases placed the control strip in the upper-right corner.
	// Move that legacy default down without overriding a placement the user has
	// deliberately saved since this migration was introduced.
	if !prefs.ChromeAnchorMigrated {
		if prefs.ChromeAnchor == chromeAnchorTopRight {
			prefs.ChromeAnchor = chromeAnchorBottomRight
		}
		prefs.ChromeAnchorMigrated = true
	}
	...
	return &App{
		...
		nativeMenuActions:   make(chan string, 16),
	}, nil
}

// RequestChromeAction safely schedules one of the quick-control actions from
// a native application menu. It is intentionally non-blocking: Cocoa invokes
// menu callbacks on the window thread, while the action itself belongs on the
// Ebiten update loop.
func (a *App) RequestChromeAction(action string) {
	select {
	case a.nativeMenuActions <- action:
	default:
	}
}
...
func (a *App) Update() error {
	if !a.nativeMenuInitialized {
		// This runs after Ebiten has initialized GLFW/Cocoa and attached the
		// macOS menu bar. Installing it earlier (before RunGame) races that
		// lifecycle and leaves no visible native menu.
		nativeui.InstallSettingsMenu(a.RequestChromeAction)
		a.nativeMenuInitialized = true
	}
	a.processNativeMenuActions()
	a.syncDiscoveryLifecycle()
...
func (a *App) processNativeMenuActions() {
	for {
		select {
		case action := <-a.nativeMenuActions:
			a.invokeChromeAction(action)
		default:
			return
		}
	}
...
func (a *App) syncUIPointer() {
	...
	if a.statsOpen && a.statsCloseButton.contains(x, y) && justPressed {
		a.statsOpen = false
		a.statsPanel = rect{}
		a.statsCloseButton = rect{}
		return
	}
	if a.statsOpen && a.statsPanel.contains(x, y) {
		// Stats are read-only, but the panel must still consume pointer input so
		// a click cannot fall through to the remote desktop underneath it.
...
func (a *App) syncMouse() {
	...
	statsPanelHovered := a.statsOpen && a.statsPanel.contains(x, y)
	if a.settingsOpen || a.pasteOpen || a.mediaOpen || a.serialConsoleOpen || statsPanelHovered || snapshot.Phase != session.PhaseConnected {
		if buttons != a.lastButtons {
			log.Trace().
				...
				Bool("stats_panel_hovered", statsPanelHovered).
...
func (a *App) syncChromeVisibility() {
	...
	if x != a.lastUIX || y != a.lastUIY {
		if hotZone.contains(x, y) || a.settingsOpen || a.pasteOpen || a.mediaOpen || a.serialConsoleOpen || a.statsOpen {
			a.revealUIFor(1600 * time.Millisecond)
		}
		a.lastUIX = x
		a.lastUIY = y
	}
	if a.settingsOpen || a.pasteOpen || a.mediaOpen || a.serialConsoleOpen || a.statsOpen {
		a.applyCursorMode()
