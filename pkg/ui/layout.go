...
func (p Panel) Draw(ctx *Context, bounds Rect) {
	// Omarchy surfaces use a soft, restrained separation rather than heavy
	// window chrome. A small radius keeps dense settings panes legible while
	// retaining the crisp, terminal-adjacent character of the controls.
	if p.Fill != nil && p.Stroke != nil {
		ctx.FillStrokedRoundedRect(bounds, 1, 6, p.Stroke, p.Fill)
	} else if p.Fill != nil {
		ctx.FillRoundedRect(bounds, 6, p.Fill)
	} else if p.Stroke != nil {
